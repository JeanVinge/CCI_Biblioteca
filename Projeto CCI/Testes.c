...
//Se o sistema for Windows
  //Adiciona determinada biblioteca, e
  //definindo comandos de limpar e esperar
#ifdef WIN32
    #include <windows.h>
...
Dados *deleta_dados(Dados *dados, int nTipo) {
    if(nTipo==1){
        ...
        free(dados->nome);
        free(dados);
        fprintf(stdout, "O primeiro registro foi deletado
 com sucesso.\n");
        getchar();
        return novo;
    }
    if(nTipo==2){
        ...
        fprintf(stdout, "O ultimo registro foi deletado
 com sucesso.\n");
        getchar();
        return aux;
    }
...
int main(void) {
 ...
 (char)
    do {
        ...
        if(chave==1){
            fprintf(stdout, "2 - Inserir no Inicio de uma
 lista\n");
            fprintf(stdout, "3 - Inserir no Fim de uma
 lista\n");
        }
        fprintf(stdout, "4 - Lista Vazia...\n");
        if(chave==1){
            fprintf(stdout, "5 - Exibir dados do primeiro
 elemento\n");
            fprintf(stdout, "6 - Exibir dados do ultimo
 elemento\n");
            fprintf(stdout, "7 - Exibir todos os valores
 da Lista\n");
            fprintf(stdout, "8 - Exibir o tamanho da
 Lista\n");
            fprintf(stdout, "9 - Eliminar primeiro
 elemento\n");
            fprintf(stdout, "a - Eliminar último
 elemento\n");
            fprintf(stdout, "b - Eliminar elemento
 buscado\n");
...
&&(escolha!='4')))
        ...
        switch(escolha) {
            ...
            case 'd':
                fprintf(stderr,"Obrigado por utilizar
 esse programa!\n");
                fprintf(stderr,"------>Terminal de
 Informação<------\n\n");
                ESPERA;
                exit(0);
                break;
